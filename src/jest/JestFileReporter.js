...
class JestFileReporter extends VerboseReporter {
  ...
  constructor(globalConfig) {
    super(globalConfig);
    this.filePathLocation = path.resolve(globalConfig.coverageDirectory, 'JestTestResults.txt');
  }
  ...
  onRunStart(aggregatedResults) {
    ...
    fs.appendFile(this.filePathLocation, `Started: ${date}\n\n`, { flag: 'a+' }, (err) => {
      if (err) {
  ...
  log(message) {
    fs.appendFile(this.filePathLocation, `${stripAnsi(message)}\n`, { flag: 'a+' }, (err) => {
      if (err) {
        console.log(`File Error -> ${err.message}`);
      }
    });
  ...
  onRunComplete() {
    ...
    fs.appendFile(this.filePathLocation, `\nEndDate: ${date}\n\n`, { flag: 'a+' }, (err) => {
      if (err) {
        console.log(`File Error -> ${err.message}`);
      }
    });
